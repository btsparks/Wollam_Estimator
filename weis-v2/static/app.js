...
function renderJobList(jobs, progress) {
    ...
    content.innerHTML = `
        ...
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px; flex-wrap: wrap;">
            ...
            <span style="font-size: 12px; color: var(--text-tertiary);">${filtered.length} job${filtered.length !== 1 ? 's' : ''}</span>
...
async function loadJobDetail(jobId) {
    ...
    try {
        const [data, recast] = await Promise.all([
...
function renderJobDetail(data) {
    ...
    content.innerHTML = `
        ...
        <div class="overview-section">
            ...
            <div class="data-grid" style="grid-template-columns: repeat(3, 1fr); margin-bottom: 16px;">
                <div class="data-item">
            ...
            <div class="data-grid" style="grid-template-columns: repeat(5, 1fr);">
                <div class="data-item">
                    <div class="data-item-label">Budget Total</div>
                    <div class="data-item-value hero">$${fmt(job.total_budget_cost)}</div>
                </div>
                <div class="data-item">
                    <div class="data-item-label">Budget Labor</div>
                    <div class="data-item-value">$${fmt(job.total_budget_labor_cost)}</div>
                </div>
                <div class="data-item">
                    <div class="data-item-label">Budget Equipment</div>
                    <div class="data-item-value">$${fmt(job.total_budget_equip_cost)}</div>
                </div>
                <div class="data-item">
                    <div class="data-item-label">Budget Material</div>
                    <div class="data-item-value">$${fmt(job.total_budget_matl_cost)}</div>
                </div>
                <div class="data-item">
                    <div class="data-item-label">Budget Subs</div>
                    <div class="data-item-value">$${fmt(job.total_budget_sub_cost)}</div>
                </div>
            </div>` : ''}
            ${state.recastCosts ? `
            <div style="font-size: 12px; font-weight: 600; color: var(--wollam-orange); text-transform: uppercase; letter-spacing: 0.03em; margin: 16px 0 8px;">Recast Actual Costs <span style="font-weight: 400; text-transform: none; font-size: 11px; color: var(--text-tertiary);">(hours x current rates)</span></div>
            <div class="data-grid" style="grid-template-columns: repeat(3, 1fr);">
                <div class="data-item" style="border-left: 3px solid var(--wollam-orange); padding-left: 12px;">
                    <div class="data-item-label">Total Recast</div>
                    <div class="data-item-value hero" style="color: var(--wollam-orange);">$${fmt(state.recastCosts.job_totals.total_cost)}</div>
                </div>
                <div class="data-item">
                    <div class="data-item-label">Recast Labor</div>
                    <div class="data-item-value">$${fmt(state.recastCosts.job_totals.labor_cost)}</div>
                </div>
                <div class="data-item">
                    <div class="data-item-label">Recast Equipment</div>
                    <div class="data-item-value">$${fmt(state.recastCosts.job_totals.equip_cost)}</div>
                </div>
            </div>` : ''}
        </div>
        ...
        <div class="doc-section" id="docSection">
            ...
            <div class="doc-upload-area" id="docUploadArea" onclick="document.getElementById('docFileInput').click()">
                <input type="file" id="docFileInput" style="display:none" accept=".pdf,.xlsx,.xls,.csv,.txt" multiple onchange="handleDocUpload(${job.job_id}, this.files)">
            ...
            <div class="interview-sidebar">
                <div class="card" style="position: sticky; top: 0; max-height: calc(100vh - 200px); overflow-y: auto;">
                    ...
                    <div class="cc-list" style="padding: 8px;" id="ccList">
                        ${codes.map(cc => `
                            <div class="cc-list-item ${state.selectedCostCode === cc.code ? 'active' : ''}"
                                 onclick="selectCostCode('${cc.code}')">
                                <span class="cc-dot ${cc.has_context ? 'has-context' : 'no-context'}"></span>
...
function renderCostCodeDetail(cc, jobId) {
    ...
    if (Object.keys(trades).length > 0) {
        crewHtml += `<div class="crew-grid">${Object.entries(trades).map(([trade, info]) =>
            `<span class="crew-tag"><strong>${trade}</strong> <span style="opacity:0.6;">${info.workers}w &middot; ${info.days}d &middot; ${fmt(info.hours)}h</span></span>`
          ).join('')}</div>`;
    }
    if (equipment.length > 0) {
        crewHtml += `<div style="margin-top: 8px;"><div class="form-label">Equipment</div><div class="crew-grid">${equipment.map(eq =>
            `<span class="crew-tag" style="background: rgba(14,165,233,0.06); border-color: rgba(14,165,233,0.15);">${eq.desc || eq.code} <span style="opacity:0.6;">(${eq.days}d / ${fmt(eq.hours)}h)</span></span>`
          ).join('')}</div></div>`;
    }
    if (!crewHtml) {
    ...
    return `
        <div class="card card-animate">
            ...
            <div class="card-body">
                ...
                ${cc.bgt_total ? `
                <!-- Budget Costs -->
                <div style="font-size: 12px; font-weight: 600; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.03em; margin-bottom: 8px;">Budget Costs</div>
                <div class="data-grid" style="grid-template-columns: repeat(5, 1fr); margin-bottom: 20px;">
                    <div class="data-item">
                        <div class="data-item-label">Total</div>
                        <div class="data-item-value" style="font-weight: 700;">$${fmt(cc.bgt_total)}</div>
                    </div>
                    <div class="data-item">
                        <div class="data-item-label">Labor</div>
                        <div class="data-item-value">$${fmt(cc.bgt_labor_cost)}</div>
                    </div>
                    <div class="data-item">
                        <div class="data-item-label">Equipment</div>
                        <div class="data-item-value">$${fmt(cc.bgt_equip_cost)}</div>
                    </div>
                    <div class="data-item">
                        <div class="data-item-label">Material</div>
                        <div class="data-item-value">$${fmt(cc.bgt_matl_cost)}</div>
                    </div>
                    <div class="data-item">
                        <div class="data-item-label">Subs</div>
                        <div class="data-item-value">$${fmt(cc.bgt_sub_cost)}</div>
                    </div>
                </div>
                ` : ''}
                ...
                ${(() => {
                    ...
                    if (!rc || rc.total_cost === 0) return '';
                    const costPerUnit = (cc.act_qty && cc.act_qty > 0) ? (rc.total_cost / cc.act_qty) : null;
                    return `
                <div style="font-size: 12px; font-weight: 600; color: var(--wollam-orange); text-transform: uppercase; letter-spacing: 0.03em; margin-bottom: 8px;">Recast Actual Costs</div>
                <div class="data-grid" style="grid-template-columns: repeat(3, 1fr); margin-bottom: 20px;">
                    <div class="data-item" style="border-left: 3px solid var(--wollam-orange); padding-left: 12px;">
                        <div class="data-item-label">Total</div>
                        <div class="data-item-value" style="font-weight: 700; color: var(--wollam-orange);">$${fmt(rc.total_cost)}</div>
                    </div>
                    <div class="data-item">
                        <div class="data-item-label">Labor</div>
                        <div class="data-item-value">$${fmt(rc.labor_cost)}</div>
                    </div>
                    <div class="data-item">
                        <div class="data-item-label">Equipment</div>
                        <div class="data-item-value">$${fmt(rc.equip_cost)}</div>
                    </div>
                </div>
                ${costPerUnit !== null ? `
                <div class="data-grid" style="grid-template-columns: repeat(2, 1fr); margin-bottom: 20px;">
                    <div class="data-item">
                        <div class="data-item-label">Recast $/Unit</div>
                        <div class="data-item-value hero" style="color: var(--wollam-orange);">$${fmtRate(costPerUnit)} <span style="font-size: 11px; color: var(--text-tertiary);">/${cc.unit || 'unit'}</span></div>
                    </div>
                    <div class="data-item">
                        <div class="data-item-label">Recast $/MH</div>
                        <div class="data-item-value">${rc.labor_hours > 0 ? '$' + fmtRate(rc.total_cost / rc.labor_hours) : '—'}</div>
                    </div>
                </div>` : ''}`;
                })()}
...
async function handleDocUpload(jobId, files) {
    ...
    if (uploadArea) {
        uploadArea.innerHTML = `
            <input type="file" id="docFileInput" style="display:none" accept=".pdf,.xlsx,.xls,.csv,.txt" multiple onchange="handleDocUpload(${jobId}, this.files)">
            <div style="color: var(--text-tertiary); font-size: 13px;">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 6px;"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                Drop files here or click to upload
                <div style="font-size: 11px; margin-top: 4px; color: var(--text-disabled);">PDF, Excel, CSV, or TXT &middot; Max 20 MB</div>
...
function renderChat() {
    ...
    document.getElementById('content').innerHTML = `
        <div class="empty-state" style="padding: 100px 24px;">
            ...
            <p>The AI Estimating Chat will be built as Feature 2. Complete PM interviews first to enrich the AI's responses with real project context.</p>
        </div>
    `;
}
